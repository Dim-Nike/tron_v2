...
def msg_user_on_ESRS(msg: str, ESRS: dict, f_key, ESRS_number: dict):
    msg_ERSR = ''
    msg_ERSR_number = ''
    for el_msg in list(msg):
        print(f'{el_msg} - {str(ESRS[el_msg])}')
        msg_ERSR += f'{str(ESRS[el_msg])}{f_key}'


    print('\n')
    for el_msg in list(msg_ERSR):
        if el_msg in ESRS_number:
            print(f'{el_msg} - {str(ESRS_number[el_msg])}')
            msg_ERSR_number += str(ESRS_number[el_msg])
        if el_msg not in ESRS_number:
            print(f'{el_msg} - {el_msg}')
            msg_ERSR_number += el_msg

    return msg_ERSR_number


def msg_ESRS_on_img(func_img, s_key, msg_esrs_number):
    msg_user_l = list(msg_esrs_number)
    key_img = 0
    img_sum_pxl_img_l = []
    msg_img = ''
    for el_pxl in func_img:
        img_sum_pxl_img_l.append(sum(el_pxl))

    if int(s_key) % 2 == 0:
        key_img += sum(img_sum_pxl_img_l) + int(s_key)
    else:
        key_img += sum(img_sum_pxl_img_l) - int(s_key)
    print(f'Это key_img - {key_img}')
    for el_msg in msg_user_l:
        if el_msg.isdigit():
            print(f'Это число el_smg - {el_msg}')
...
image_path = 'C:/Users/User/Desktop/структура шаблона.png'
img_pxl = get_image_pixels(image_path=image_path)
...
msg_ERSR = msg_user_on_ESRS(msg_user, ESRS, f_key, ERSR_number)
print(msg_ESRS_on_img(img_pxl, s_key, msg_ERSR))
