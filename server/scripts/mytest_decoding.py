...
for i, el_msg in enumerate(test_de_msg):
    new_msg = de_msg(en_msg=el_msg, f_key=test_f_key[0], s_key=test_s_key[0], noise_l=noise_list,
...
print('Проверка на корректность первого ключа')
for i, el_f_key in enumerate(test_f_key):
    new_msg = de_msg(en_msg=test_de_f_key[i], f_key=el_f_key, s_key=test_s_key[0], noise_l=noise_list,
           img_path=r'C:\Users\User\Desktop\ИП Хорошко М.png',  esrs=ESRS)
...
print('Проверка на корректность второго ключа')
for i, el_s_key in enumerate(test_s_key):
    new_msg = de_msg(en_msg=test_de_s_key[i], f_key=test_f_key[0], s_key=el_s_key, noise_l=noise_list,
           img_path=r'C:\Users\User\Desktop\ИП Хорошко М.png',  esrs=ESRS)
    if test_msg[0] == new_msg:
